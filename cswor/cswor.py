...
def get_martingale(x, S_t, t, DM_alpha, T_null):
    '''
...
def cs_from_martingale_2D(x, N, mart_fn, mart_fn_og, n=None, alpha=0.05, search_step=1, running_intersection=False):
    '''
    ...
    for i in range(len(x)):
        t += n[i]
        X = np.array([x[i], n[i] - x[i]])
        S_t += X
        np.random.seed(42)
        pivot = np.random.rand() * (upper[i-1]-lower[i-1])+lower[i-1]
        f = mart_fn(X, S_t, t, pivot)
...
def BBHG_confseq(x, N, BB_alpha, BB_beta, n=None, alpha=0.05,
    ...
    def mart_fn_og(x, m): return DMHG_martingale(DM_x,
                                                 [BB_alpha, BB_beta],
                                                 [int(N*m), N-int(N*m)])

    def mart_fn(x, S_t, t, m): return get_martingale(
        tuple(x), tuple(S_t), t, (BB_alpha, BB_beta), (int(N*m), N-int(N*m)))

    l_01, u_01 = cs_from_martingale_2D(x, N, mart_fn, mart_fn_og, n=n, alpha=alpha,
...
def plot_2party_election(x, n, N, BB_alpha, BB_beta, Tb_true, Tb_null,
    ...
    plt.plot(t, np.ones(len(t)) * Tb_true, '--', color='green',
             label='True number of votes: {}'.format(Tb_true))
    plt.fill_between(times, CIs_lower_false, CIs_upper_false, color='red',
                     alpha=alpha_false, label='Full Confidence Sequence')
    if running_intersection is True:
        plt.fill_between(times, CIs_lower_true, CIs_upper_true,
                         color='orange', alpha=alpha_true, label='Running Intersection')
    plt.legend()
    return fig1, fig2


class Confseq3D:
    def __init__(self, votes, N, DM_alpha, alpha, fineness=1):
        ...
        # self.df.columns = np.minimum(self.N, np.floor(self.df.columns / fineness))
        # self.df.index = self.df.columns
        self.df = np.zeros((self.N_cmp,) * (3 - 1))
    ...
    def update_from_S_t(self, idx):
        # Reset margins
        self.margin_lower = -self.N
        self.margin_upper = self.N
        for i in range(idx):
            if i == len(self.votes[0]) - 1:
                l = self.votes.sum(axis=1)[1]
                print(i, l, l)
                return
            CIs_lower_a, CIs_upper_a = BBHG_confseq(self.votes[1,:i+1], self.N-self.votes[0, i], 1, 1, alpha=self.alpha, running_intersection=True)
            l = int(CIs_lower_a[-1])
            u = int(CIs_upper_a[-1])
            self.margin_lower = max(self.margin_lower, l)
            self.margin_upper = min(self.margin_upper, u)
            print(i, l, u)
            # print(i, self.get_margin_confint())
            # for total_2 in range(self.N-total_1+1):
            #     total_3 = self.N - total_1 - total_2
            #     pval = pval_from_martingale(
            #         DMHG_martingale(x,
            #                         self.DM_alpha,
            #                         (total_1, total_2, total_3)))
            #     self.df[total_2][total_1] = pval > self.alpha
...
def predmix_empbern_wor(x, N, lambdas=None,
    ...
    margin = (np.cumsum(np.power(c/2, -2) * np.power(x - mu_hat_tminus1, 2)
                        * psi) + np.log(2/alpha)) / np.cumsum(lambdas*(1+Wstar))
...
def predmix_hoeffding_wor(x, N, lambdas=None,
    ...
    if lambdas is None:
        ...
        lambdas[np.logical_or(np.isnan(lambdas), lambdas == math.inf)] = 0
        lambdas = np.minimum(
            1/np.sqrt(np.power(upper_bd-lower_bd, 2)), lambdas)
...
def empbern_wor_ci_seq(x, N, times, lower_bd=0,
    ...
    u, array-like of reals
        Sequence of upper confidence intervals
    '''
    def cs_fn(x): return empbern_wor_ci(x, N,
                                        lower_bd,
                                        upper_bd,
                                        alpha)
